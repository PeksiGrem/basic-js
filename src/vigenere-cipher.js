...
class VigenereCipheringMachine {
  ...
  decrypt(str, key) {
    ...
    let resStr = '';
    let j = 0;
    let keyArr = (key.repeat(Math.ceil(str.length / key.length)).match(new RegExp('.{1,' + key.length + '}', 'g'))).join('');
    keyArr = keyArr.toLowerCase().split('');
    for (let i = 0; i < strArr.length; i++) {
      ci = strArr[i].charCodeAt(0) - 96;
      ki = keyArr[i - j].charCodeAt(0) - 96;
      if (!strArr[i].match(/[A-Z]/gi)) {
        result.push(strArr[i]);
        j++;
      } else {
